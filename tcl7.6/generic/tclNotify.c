...
/*
 * Modified by AOL to support per-thread notifiers.
 */

static const char *RCSID = "@(#) $Header: /Users/dossy/Desktop/cvs/aolserver/tcl7.6/generic/Attic/tclNotify.c,v 1.1.1.1 2000/05/02 13:48:28 kriston Exp $, compiled: " __DATE__ " " __TIME__;
 ...
 */
    ...
    int blockTimeSet;		/* 0 means there is no maximum block
				 * time:  block forever. */
    Tcl_Time blockTime;		/* If nf->blockTimeSet is 1, gives the
...
static NotifyData *		GetNotifyData(void);
...
Tcl_QueueEvent(evPtr, position)
    ...
    Tcl_QueuePosition position;	/* One of TCL_QUEUE_TAIL, TCL_QUEUE_HEAD,
				 * TCL_QUEUE_MARK. */
{
    NotifyData *ndPtr = GetNotifyData();
	...
	evPtr->nextPtr = NULL;
	if (ndPtr->firstEventPtr == NULL) {
	    ndPtr->firstEventPtr = evPtr;
	} else {
	    ndPtr->lastEventPtr->nextPtr = evPtr;
	}
	ndPtr->lastEventPtr = evPtr;
    } else if (position == TCL_QUEUE_HEAD) {
	/*
	 * Push the event on the head of the queue.
	 */

	evPtr->nextPtr = ndPtr->firstEventPtr;
	if (ndPtr->firstEventPtr == NULL) {
	    ndPtr->lastEventPtr = evPtr;
	}	    
	ndPtr->firstEventPtr = evPtr;
	...
	if (ndPtr->markerEventPtr == NULL) {
	    evPtr->nextPtr = ndPtr->firstEventPtr;
	    ndPtr->firstEventPtr = evPtr;
	} else {
	    evPtr->nextPtr = ndPtr->markerEventPtr->nextPtr;
	    ndPtr->markerEventPtr->nextPtr = evPtr;
	}
	ndPtr->markerEventPtr = evPtr;
    ...
    ClientData clientData;    		/* type-specific data. */
{
    ...
    for (prevPtr = (Tcl_Event *) NULL, evPtr = ndPtr->firstEventPtr;
             evPtr != (Tcl_Event *) NULL;
             ) {
        if ((*proc) (evPtr, clientData) == 1) {
            if (ndPtr->firstEventPtr == evPtr) {
                ndPtr->firstEventPtr = evPtr->nextPtr;
                if (evPtr->nextPtr == (Tcl_Event *) NULL) {
                    ndPtr->lastEventPtr = (Tcl_Event *) NULL;
                }
            } else {
				 ...
				 * later. */
{
    ...
    for (evPtr = ndPtr->firstEventPtr; evPtr != NULL; evPtr = evPtr->nextPtr) {
	/*
	...
	if ((proc != NULL) && (*proc)(evPtr, flags)) {
		...
		ndPtr->firstEventPtr = evPtr->nextPtr;
		if (evPtr->nextPtr == NULL) {
		    ndPtr->lastEventPtr = NULL;
		}
		if (ndPtr->markerEventPtr == evPtr) {
		    ndPtr->markerEventPtr = NULL;
		}
	    } else {
		for (prevPtr = ndPtr->firstEventPtr; prevPtr->nextPtr != evPtr;
			prevPtr = prevPtr->nextPtr) {
		    /* Empty loop body. */
		}
		prevPtr->nextPtr = evPtr->nextPtr;
		if (evPtr->nextPtr == NULL) {
		    ndPtr->lastEventPtr = prevPtr;
		}
		if (ndPtr->markerEventPtr == evPtr) {
		    ndPtr->markerEventPtr = prevPtr;
		}
	    }
	    ckfree((char *) evPtr);
	    return 1;
				 ...
				 * event notifier. */
{
    ...
    if (!ndPtr->blockTimeSet || (timePtr->sec < ndPtr->blockTime.sec)
	    || ((timePtr->sec == ndPtr->blockTime.sec)
	    && (timePtr->usec < ndPtr->blockTime.usec))) {
	ndPtr->blockTime = *timePtr;
	ndPtr->blockTimeSet = 1;
    }
}

/*
				 ...
				 * others defined by event sources. */
{
	...
	if ((flags & TCL_DONT_WAIT) ||
	    ...
	    ndPtr->blockTime.sec = 0;
	    ndPtr->blockTime.usec = 0;
	    timePtr = &ndPtr->blockTime;
	} else if (ndPtr->blockTimeSet) {
	    timePtr = &ndPtr->blockTime;
	} else {
	    timePtr = NULL;
	}
 ...
 * GetNotifyData --
 *
 *	Return the per-thread notify data.
 *
 * Results:
 *	Return pointer to NotifyData.
 *
 * Side effects:
 *	None.
 *
...
GetNotifyData(void)
{
    ...
    ndPtr = Ns_TlsGet(&tls);
    if (ndPtr == NULL) {
	ndPtr = (NotifyData *) ns_calloc(1, sizeof(NotifyData));
	Ns_TlsSet(&tls, ndPtr);
    }
    return ndPtr;
}
