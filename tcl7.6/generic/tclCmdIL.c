...
/*
 * Modified by AOL so support per-thread lsort context.
 */

static const char *RCSID = "@(#) $Header: /Users/dossy/Desktop/cvs/aolserver/tcl7.6/generic/Attic/tclCmdIL.c,v 1.1.1.1 2000/05/02 13:48:26 kriston Exp $, compiled: " __DATE__ " " __TIME__;
...
typedef struct {
    Tcl_Interp *interp;			/* Interpreter for "lsort" command. 
					 * NULL means no lsort is active. */
    enum {ASCII, INTEGER, REAL, COMMAND} mode;
					/* Mode for sorting: compare as strings,
					 * compare as numbers, or call
					 * user-defined command for
					 * comparison. */
    Tcl_DString cmd;			/* Holds command if mode is COMMAND.
					 * pre-initialized to hold base of
					 * command. */
    int increasing;			/* 0 means sort in decreasing order,
					 * 1 means increasing order. */
    int code;				/* Anything other than TCL_OK means a
					 * problem occurred while sorting; this
					 * executing a comparison command, so
					 * the sort was aborted. */
    jmp_buf env;			/* Jump buf to protect against a possibly
					 * recursive qsort() stack overflow. */
} SortData;
...
static int		SortCompareProc _ANSI_ARGS_((CONST VOID *first,
			    CONST VOID *second));
static void		FreeSortData(void *arg);
static SortData *	GetSortData(void);
    ...
    char **argv;			/* Argument strings. */
{
    ...
    if (sdPtr->mode == COMMAND) {
	Tcl_DStringInit(&sdPtr->cmd);
	Tcl_DStringAppend(&sdPtr->cmd, command, -1);
    }
    ...
    if (sdPtr->code == TCL_OK) {
	Tcl_ResetResult(interp);
	interp->result = Tcl_Merge(listArgc, listArgv);
	interp->freeProc = TCL_DYNAMIC;
    }
    if (sdPtr->mode == COMMAND) {
	Tcl_DStringFree(&sdPtr->cmd);
    }
    ckfree((char *) listArgv);
    ...
    CONST VOID *first, *second;		/* Elements to be compared. */
{
    ...
    } else if (sdPtr->mode == INTEGER) {
	int a, b;

	if ((Tcl_GetInt(sdPtr->interp, firstString, &a) != TCL_OK)
		|| (Tcl_GetInt(sdPtr->interp, secondString, &b) != TCL_OK)) {
	    Tcl_AddErrorInfo(sdPtr->interp,
    ...
    } else if (sdPtr->mode == REAL) {
	double a, b;

	if ((Tcl_GetDouble(sdPtr->interp, firstString, &a) != TCL_OK)
		|| (Tcl_GetDouble(sdPtr->interp, secondString, &b) != TCL_OK)) {
	    Tcl_AddErrorInfo(sdPtr->interp,
	...
	oldLength = Tcl_DStringLength(&sdPtr->cmd);
	Tcl_DStringAppendElement(&sdPtr->cmd, firstString);
	Tcl_DStringAppendElement(&sdPtr->cmd, secondString);
	sdPtr->code = Tcl_Eval(sdPtr->interp, Tcl_DStringValue(&sdPtr->cmd));
	Tcl_DStringTrunc(&sdPtr->cmd, oldLength);
	if (sdPtr->code != TCL_OK) {
	    Tcl_AddErrorInfo(sdPtr->interp,
	...
	order = strtol(sdPtr->interp->result, &end, 0);
	if ((end == sdPtr->interp->result) || (*end != 0)) {
	    Tcl_ResetResult(sdPtr->interp);
	    Tcl_AppendResult(sdPtr->interp,
...
GetSortData(void)
{
	...
	Ns_MasterLock();
	if (tls == NULL) {
	    Ns_TlsAlloc(&tls, FreeSortData);
	}
	Ns_MasterUnlock();
    }
    sdPtr = (SortData *) Ns_TlsGet(&tls);
    if (sdPtr == NULL) {
	sdPtr = (SortData *) ns_calloc(1, sizeof(SortData));
	Tcl_DStringInit(&sdPtr->cmd);
	Ns_TlsSet(&tls, sdPtr);
    }
    return sdPtr;
 ...
 * FreeSortData --
 *
 *	Thread local storage cleanup callback for per-thread SortData.
 *
 * Results:
 *	None.
 *
...
static void
FreeSortData(void *arg)
{
    SortData *sdPtr = (SortData *) arg;
