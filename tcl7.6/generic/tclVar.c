...
/*
 * Modified by AOL to support shared variables and global variable purging.
 */

static const char *RCSID = "@(#) $Header: /Users/dossy/Desktop/cvs/aolserver/tcl7.6/generic/Attic/tclVar.c,v 1.1.1.1 2000/05/02 13:48:28 kriston Exp $, compiled: " __DATE__ " " __TIME__;
...
static char *noShare =	    	"unsupported for shared variables";
static char *noShareElement =	"must share whole arrays";
static char *localExists =	"variable exists";
...
static Var *		NewVar _ANSI_ARGS_((Ns_Mutex *lockPtr));
static ArraySearch *	ParseSearchId _ANSI_ARGS_((Tcl_Interp *interp,
			    Var *varPtr, char *varName, char *string));
static void		VarErrMsg _ANSI_ARGS_((Tcl_Interp *interp,
			    char *part1, char *part2, char *operation,
			    char *reason));
static int		ArrayCmd2 _ANSI_ARGS_((ClientData dummy,
    			    Tcl_Interp *interp, int argc, char **argv,
    			    Ns_Mutex **lockPtrPtr));
static int		MakeUpvar2 _ANSI_ARGS_((Interp *iPtr,
			    CallFrame *framePtr, char *otherP1,
			    char *otherP2, char *myName, int flags,
			    Ns_Mutex **lockPtrPtr));

static char		*GetPrivateCopy(Var *varPtr);

#define VarValue(varPtr) ((varPtr)->lockPtr ? GetPrivateCopy((varPtr)) \
				: (varPtr)->value.string)
				 ...
				 * this is set to NULL. */
{
    ...
    *arrayPtrPtr = NULL;
    ...
    /*
     * If this is a shared variable, aquire the lock here;
     * the caller is responsible for checking varPtr->lock
     * on the return and releasing it after possibly copying
     * the value. Note that this works even if an array
     * element if fetch below because the varPtr->lock
     * for an array element is always the same as the
     * parent array's lock.
     */

    if (varPtr->lockPtr != NULL) {
    	Ns_MutexLock(varPtr->lockPtr);
    }
    if (elName == NULL) {
	return varPtr;
    }
	...
	if (!(create & CRT_PART1)) {
		...
		VarErrMsg(interp, part1, part2, msg, noSuchVar);
    	    ...
    	    if (varPtr->lockPtr) Ns_MutexUnlock(varPtr->lockPtr);
	    return NULL;
	}
	...
	*closeParen = 0;
	...
	if (new) {
	    ...
	    varPtr = NewVar((*arrayPtrPtr)->lockPtr);
	    Tcl_SetHashValue(hPtr, varPtr);
	    varPtr->hPtr = hPtr;
	}
    } else {
 ...
 */
	...
	/* ARGSUSED */
static int
ArrayCmd2(dummy, interp, argc, argv, lockPtrPtr)
    ClientData dummy;			/* Not used. */
    register Tcl_Interp *interp;	/* Current interpreter. */
    int argc;				/* Number of arguments. */
    char **argv;			/* Argument strings. */
    Ns_Mutex **lockPtrPtr;
{
	...
	if (*lockPtrPtr) Ns_MutexUnlock(*lockPtrPtr);
	...
	if (*lockPtrPtr) Ns_MutexLock(*lockPtrPtr);
	setDone:
	...
	ArraySearch *searchPtr;
    	...
    	if (*lockPtrPtr) {
    	    VarErrMsg(interp, argv[2], NULL, argv[1], noShare);
    	    return TCL_ERROR;
    	}
	if (argc != 3) {
	    Tcl_AppendResult(interp, "wrong # args: should be \"",
 ...
 * Tcl_ArrayCmd --
 *
 *	Wrapper around ArrayCmd2 which releases any aquired lock.
 *
 * Results:
 *	Result of ArrayCmd2.
 *
 * Side effects:
 *	Side effects of ArrayCmd2.
 *
 *----------------------------------------------------------------------
 */
    ...
    char **argv;			/* Argument strings. */
{
    ...
    result = ArrayCmd2(dummy, interp, argc, argv, &lockPtr);
    if (lockPtr) Ns_MutexUnlock(lockPtr);
    return result;
}

/*
...
static int
MakeUpvar2(iPtr, framePtr, otherP1, otherP2, myName, flags, lockPtrPtr)
 ...
 * MakeUpvar --
 *
 *	Wrapper around MakeUpvar2 which releases any aquired lock.
 *
 * Results:
 *	Result of MakeUpvar2.
 *
 * Side effects:
 *	Side effects of MakeUpvar2.
 *
 *----------------------------------------------------------------------
 */
				 ...
				 * myName. */
{
    ...
    result = MakeUpvar2(iPtr, framePtr, otherP1, otherP2, myName, flags, &lockPtr);
    if (lockPtr) Ns_MutexUnlock(lockPtr);
    return result;
}

...
static Var *
NewVar(lockPtr)
    Ns_Mutex *lockPtr;	/* Variables shared lock (if any). */
{
    register Var *varPtr;
...
/*
 * AOLserver shared variable support.
 */

static Tcl_HashTable shareTable;/* Table of shared variables. */
static Ns_Mutex shareLock;	/* Lock around access to shareTable. */
static Ns_Tls shareTls;		/* TLS for shared variable value copies. */
 ...
 * GetPrivateCopy -
 *
 *	This procedure copies the current value of a shared variable
 *	to thread local storage and then releases the lock.  It
 *	is only called by the VarValue macro.
 *
 * Results:
 *	A pointer to an allocated string in thread local storage.
 *
 * Side effects:
 *	The shared variable lock is released.
 *
 *----------------------------------------------------------------------
 */

static char *
GetPrivateCopy(Var *varPtr)
{
    char *privateCopy;

    privateCopy = (char *) Ns_TlsGet(&shareTls);

    /*
     * Free any previous privateCopy.
     */
    ...
    /*
     * Copy the new value and store it's pointer in thread local storage.
     */
    
    privateCopy = ns_strdup(varPtr->value.string);
    Ns_TlsSet(&shareTls, privateCopy);

    /*
     * Release the shared variable lock.  See the comment in ShareVar
     * below for details on why the lock is released here.
     */

    Ns_MutexUnlock(varPtr->lockPtr);
    return privateCopy;
}

/*
 ...
 * ShareVar -
 *
 *	This procedure is invoked to actually share a variable.
 *
 * Results:
 *	TCL_OK if the variable could be shared, TCL_ERROR otherwise.
 *
 * Side effects:
 *	Very similar to "MakeUpvar" for regular variables.
 *
 *----------------------------------------------------------------------
 */

int
NsTclShareVar(Tcl_Interp *interp, char *part1)
{
    Tcl_HashTable *tablePtr;
    Tcl_HashEntry *hPtr;
    int new;
    Var *varPtr, *upvarPtr;
    register char *s = part1;
    Interp *iPtr = (Interp *) interp;

    if (shareTls == NULL) {
	Ns_MasterLock();
	if (shareTls == NULL) {
	    Ns_TlsAlloc(&shareTls, Ns_Free);
    ...
    if ((s = strchr(part1, '(')) != NULL && (strchr(s, ')') != NULL)) {
	VarErrMsg(interp, part1, NULL, "share", noShareElement);
    	return TCL_ERROR;
    }

    /*
     * Create the local variable entry and ensure it's currently undefined.
     */
	...
	varPtr = (Var *) Tcl_GetHashValue(hPtr);
	if (!(varPtr->flags & VAR_UNDEFINED)) {
	    VarErrMsg(interp, part1, NULL, "share", localExists);
    	    return TCL_ERROR;
	}
    }
     ...
     */
    Ns_MutexLock(&shareLock);
    hPtr = Tcl_CreateHashEntry(&shareTable, part1, &new);
    if (!new) {
    	upvarPtr = Tcl_GetHashValue(hPtr);
    } else {
	 ...
	 */
    	...
    	upvarPtr = NewVar((Ns_Mutex *) ckalloc(sizeof(Ns_Mutex)));
    	Ns_MutexInit(upvarPtr->lockPtr);
    	upvarPtr->hPtr = hPtr;
    	Tcl_SetHashValue(hPtr, upvarPtr);
	++upvarPtr->refCount;
    }
    Ns_MutexUnlock(&shareLock);

    /*
     * Increment the refCount once again to ensure the variable
     * will not be deleted when the local variable is cleaned up.
     */

    Ns_MutexLock(upvarPtr->lockPtr);
    ++upvarPtr->refCount;
    Ns_MutexUnlock(upvarPtr->lockPtr);

    varPtr->flags |= VAR_UPVAR;
    varPtr->value.upvarPtr = upvarPtr;
    return TCL_OK;
}
