...
/*
 * Modified by AOL to support per-thread channel I/O and add the
 * NsTclCloseAllFiles() routine.
 */

static const char *RCSID = "@(#) $Header: /Users/dossy/Desktop/cvs/aolserver/tcl7.6/generic/Attic/tclIO.c,v 1.1.1.1 2000/05/02 13:48:27 kriston Exp $, compiled: " __DATE__ " " __TIME__;
...
/*
 * The following structure maintains per-thread channel I/O data.
 */
...
typedef struct IOData {
    NextChannelHandler *nestedHandlerPtr;
					 ...
					 * ChannelHandlerEventProc
					 * invocations. */
    Channel *firstChanPtr;		/* List of all channels currently
					 * open. */
} IOData;
    ...
    int *fileUsedPtr;
{
    ...
    IOData *idPtr = GetIOData();
    
    *fileUsedPtr = 0;
    for (chanPtr = idPtr->firstChanPtr;
    ...
    ClientData clientData;		/* NULL - unused. */
{
    ...
    for (chanPtr = idPtr->firstChanPtr; chanPtr != (Channel *) NULL;
             chanPtr = nextChanPtr) {
                                 ...
                                 * if the channel is readable, writable. */
{
...
#else
    /* NB:  AOLserver assumes LF - screw Windows and the Mac. */
    ...
    /*
     * Link the channel into the list of all channels.
     */

    chanPtr->nextChanPtr = idPtr->firstChanPtr;
    idPtr->firstChanPtr = chanPtr;
    ...
    int errorCode;			/* Status of operation so far. */
{
    ...
    if (chanPtr == idPtr->firstChanPtr) {
        idPtr->firstChanPtr = chanPtr->nextChanPtr;
    } else {
        for (prevChanPtr = idPtr->firstChanPtr;
                                 ...
                                 * handle, such as TCL_FILE_EVENTS. */
{
    ...
    nh.nestedHandlerPtr = idPtr->nestedHandlerPtr;
    idPtr->nestedHandlerPtr = &nh;
    ...
    idPtr->nestedHandlerPtr = nh.nestedHandlerPtr;
    ...
    
{
    ...
    for (nhPtr = idPtr->nestedHandlerPtr;
...
Tcl_GetChannelNames(Tcl_Interp *interp)
{
    ...
    chanPtr = idPtr->firstChanPtr;
    while (chanPtr != NULL) {
        if (chanPtr == (Channel *) stdinChannel) {
	    name = "stdin";
	} else if (chanPtr == (Channel *) stdoutChannel) {
	    name = "stdout";
	} else if (chanPtr == (Channel *) stderrChannel) {
	    name = "stderr";
	} else {
	    name = chanPtr->channelName;
	}
 ...
 * GetIOData --
 *
 *	Return the per-thread channel I/O data structure.
 *
 * Results:
 *	Pointer to IOData.
 *
...
GetIOData(void)
{
    ...
    idPtr = (IOData *) Ns_TlsGet(&tls);
    if (idPtr == NULL) {
	idPtr = (IOData *) ns_calloc(1, sizeof(IOData));
	Ns_TlsSet(&tls, idPtr);
    }
    return idPtr;
}
