...
/*
 * Modified by AOL to support per-thread events.
 */

static const char *RCSID = "@(#) $Header: /Users/dossy/Desktop/cvs/aolserver/tcl7.6/generic/Attic/tclEvent.c,v 1.1.1.1 2000/05/02 13:48:26 kriston Exp $, compiled: " __DATE__ " " __TIME__;
...
/*
 * The following structure and function maintains per-thread event data.
 */

typedef struct EventData {
    FileHandler *firstFileHandlerPtr;
				/* List of all file handlers. */
    int fileEventSourceCreated; /* Zero means that the file event source
				 * hasn't been registerd with the Tcl
				 * notifier yet. */
    TimerHandler *firstTimerHandlerPtr; /* First event in queue. */
    int timerEventSourceCreated;	/* 0 means that the timer event source
					 * hasn't yet been registered with the
					 * Tcl notifier. */
    TimerHandler *firstModalHandlerPtr; /* The information below describes
					 * a stack of modal timeouts
					 * managed by Tcl_CreateModalTimer
					 * and Tcl_DeleteModalTimer.
					 * Only the first element in the
					 * list is used at any given time. */
...
static char *		VwaitVarProc _ANSI_ARGS_((ClientData clientData,
			    ...
			    int flags));
static EventData *	GetEventData(void);
    ...
    ClientData clientData;	/* Arbitrary data to pass to proc. */
{
    ...
    EventData *evPtr = GetEventData();

    if (!evPtr->fileEventSourceCreated) {
	evPtr->fileEventSourceCreated = 1;
    ...
    for (filePtr = evPtr->firstFileHandlerPtr; filePtr != NULL;
	...
	filePtr->file = file;
	filePtr->nextPtr = evPtr->firstFileHandlerPtr;
	evPtr->firstFileHandlerPtr = filePtr;
    }
				 ...
				 * callback procedure. */
{
    ...
    for (prevPtr = NULL, filePtr = evPtr->firstFileHandlerPtr; ;
    ...
    if (prevPtr == NULL) {
	evPtr->firstFileHandlerPtr = filePtr->nextPtr;
    } else {
				 ...
				 * handle, such as TCL_FILE_EVENTS. */
{
    ...
    for (filePtr = edPtr->firstFileHandlerPtr; filePtr != NULL;
	    filePtr = filePtr->nextPtr) {
    ...
    ClientData clientData;	/* Arbitrary data to pass to proc. */
{
    ...
    if (!edPtr->timerEventSourceCreated) {
	edPtr->timerEventSourceCreated = 1;
    ...
    for (tPtr2 = edPtr->firstTimerHandlerPtr, prevPtr = NULL; tPtr2 != NULL;
    ...
    if (prevPtr == NULL) {
	edPtr->firstTimerHandlerPtr = timerHandlerPtr;
    } else {
				 ...
				 * Tcl_DeleteTimerHandler. */
{
    ...
    for (timerHandlerPtr = edPtr->firstTimerHandlerPtr, prevPtr = NULL;
	...
	if (prevPtr == NULL) {
	    edPtr->firstTimerHandlerPtr = timerHandlerPtr->nextPtr;
	} else {
    ...
    ClientData clientData;	/* Arbitrary data to pass to proc. */
{
    ...
    timerHandlerPtr->nextPtr = edPtr->firstModalHandlerPtr;
    edPtr->firstModalHandlerPtr = timerHandlerPtr;
}

/*
 *--------------------------------------------------------------
    ...
    ClientData clientData;	/* Arbitrary data to pass to proc. */
{
    ...
    timerHandlerPtr = edPtr->firstModalHandlerPtr;
    edPtr->firstModalHandlerPtr = timerHandlerPtr->nextPtr;
					 ...
					 * consider modal timers. */
{
    ...
    timerHandlerPtr = edPtr->firstTimerHandlerPtr;
    if (!(flags & TCL_TIMER_EVENTS)) {
	timerHandlerPtr = NULL;
    }
    if (timerHandlerPtr != NULL) {
	tPtr2 = edPtr->firstModalHandlerPtr;
	if (tPtr2 != NULL) {
				 ...
				 * handle, such as TCL_FILE_EVENTS. */
{
    ...
    timerHandlerPtr = edPtr->firstModalHandlerPtr;
    if (edPtr->firstModalHandlerPtr != NULL) {
    ...
    ClientData clientData;	/* Arbitrary value to pass to proc. */
{
    ...
    idlePtr->generation = edPtr->idleGeneration;
    idlePtr->nextPtr = NULL;
    if (edPtr->lastIdlePtr == NULL) {
	edPtr->idleList = idlePtr;
    } else {
	edPtr->lastIdlePtr->nextPtr = idlePtr;
    }
    edPtr->lastIdlePtr = idlePtr;
}

/*
 *----------------------------------------------------------------------
    ...
    ClientData clientData;	/* Arbitrary value to pass to proc. */
{
    ...
    for (prevPtr = NULL, idlePtr = edPtr->idleList; idlePtr != NULL;
...
{
    EventData *edPtr = GetEventData();
    return (edPtr->idleList == NULL) ? 0 : 1;
}

/*
 *----------------------------------------------------------------------
...
TclServiceIdle()
{
    ...
    if (edPtr->idleList == NULL) {
	return 0;
    }
    
    foundIdle = 0;
    oldGeneration = edPtr->idleGeneration;
    edPtr->idleGeneration++;
    ...
    for (idlePtr = edPtr->idleList;
	    ((idlePtr != NULL)
		    && ((oldGeneration - idlePtr->generation) >= 0));
	    idlePtr = edPtr->idleList) {
	edPtr->idleList = idlePtr->nextPtr;
	if (edPtr->idleList == NULL) {
	    edPtr->lastIdlePtr = NULL;
	}
	foundIdle = 1;
 ...
 * GetEventData --
 *
 *	Return the per-thread event data.
 *
 * Results:
...
GetEventData(void)
{
    ...
    edPtr = (EventData *) Ns_TlsGet(&tls);
    if (edPtr == NULL) {
	edPtr = (EventData *) ns_calloc(1, sizeof(EventData));
	Ns_TlsSet(&tls, edPtr);
    }
    return edPtr;
}   
