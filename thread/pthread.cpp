...
Ns_CondWait(Ns_Cond *condPtr, Ns_Mutex *mutexPtr)
{
     ...
     * we're not sure why ETIME is being returned, we allow
...
Ns_CondTimedWait(Ns_Cond *condPtr, Ns_Mutex *mutexPtr, Ns_Time *timePtr)
{
...
#ifdef HAVE_COND_EINTR
    /* NB: EINTR can be generated by the GDB debugger on Linux. */
    do {
     ...
     * As above, this should be ok as properly written condition code
     * should tolerate the wakeup.
     */
    if (err == ETIME) {
	err = 0;
    }
...
NsThreadCreate(Thread *thrPtr)
{
    ...
    if (err != 0) {
        NsThreadFatal("Ns_ThreadCreate", "pthread_attr_setstacksize", err);
    }
    ...
    if (err != 0) {
        NsThreadFatal("Ns_ThreadCreate", "pthread_attr_setscope",err);
    }
#endif
    err = pthread_create(&thr, &attr, (void *(*)(void *)) NsThreadMain, thrPtr);
    if (err != 0) {
        NsThreadFatal("Ns_ThreadCreate", "pthread_create", err);
    }
    err = pthread_detach(thr);
    if (err != 0) {
        NsThreadFatal("Ns_ThreadCreate", "pthread_detach", err);
    }
    err = pthread_attr_destroy(&attr);
    if (err != 0) {
        NsThreadFatal("Ns_ThreadCreate", "pthread_attr_destroy", err);
    }
}
